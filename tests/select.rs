...
/// Tip `SelectCompleted` must be observable via ordinary `drive()` recovery when an
/// unscoped timer would otherwise short-circuit (DriveRun path).
struct SelectCompletedBesideOpenWaitRuntime {
    race_phase: AtomicBool,
}

#[async_trait]
impl FlowRuntime for SelectCompletedBesideOpenWaitRuntime {
    async fn run_workflow(
        &self,
        invocation: WorkflowInvocation,
    ) -> a3s_flow::Result<RuntimeCommand> {
        ...
        let select_opened = ctx.history().iter().any(|envelope| {
            matches!(
                &envelope.event,
                FlowEvent::SelectCreated { select_id, .. } if select_id == "race"
            )
        });
        if self.race_phase.load(Ordering::SeqCst) && !select_opened {
            let soon = Utc::now() - Duration::seconds(1);
            let later = Utc::now() + Duration::hours(1);
    ...
    async fn run_step(&self, _invocation: a3s_flow::StepInvocation) -> a3s_flow::Result<JsonValue> {
        unreachable!("select-completed-beside-wait runtime does not schedule steps")
    }
...
struct CrashAfterSelectCompletedStore {
    inner: InMemoryEventStore,
    armed: AtomicBool,
}

impl CrashAfterSelectCompletedStore {
    fn new() -> Self {
        Self {
            inner: InMemoryEventStore::new(),
...
#[async_trait]
impl FlowEventStore for CrashAfterSelectCompletedStore {
    async fn append(&self, run_id: &str, event: FlowEvent) -> a3s_flow::Result<FlowEventEnvelope> {
        self.inner.append(run_id, event).await
    }
    ...
    async fn append_if_sequence(
        ...
        {
            return Err(FlowError::Store(
                "injected crash after SelectCompleted before observation drive".into(),
            ));
        }
        Ok(envelope)
    }
...
#[tokio::test]
async fn select_completed_recovery_wakes_via_drive_while_unscoped_timer_is_open() {
    let store = Arc::new(CrashAfterSelectCompletedStore::new());
    let runtime = Arc::new(SelectCompletedBesideOpenWaitRuntime {
        race_phase: AtomicBool::new(false),
    });
    let engine = FlowEngine::new(store.clone(), runtime.clone());
    engine
        .start_with_id(
            "select-completed-drive",
            select_spec().with_update("arm"),
            json!({}),
        )
        .await
        .unwrap();
    engine.drive("select-completed-drive").await.unwrap();

    runtime.race_phase.store(true, Ordering::SeqCst);
    engine
        .apply_update(
            "select-completed-drive",
    ...
    let interrupted = engine
        .resume_wait("select-completed-drive", "fast")
        .await
        .expect_err("crash after durable SelectCompleted must interrupt");
    assert!(matches!(interrupted, FlowError::Store(_)));

    let mid = engine.snapshot("select-completed-drive").await.unwrap();
    assert!(!mid.status.is_terminal());
    assert!(mid.select("race").unwrap().is_completed());
    assert_eq!(mid.waits["outer"].status, WaitStatus::Waiting);

    let recovered = engine.drive("select-completed-drive").await.unwrap();
    assert_eq!(
        recovered.output,
        Some(json!({ "winner": "fast" })),
        "drive must observe tip SelectCompleted beside an open unscoped timer"
    );
    assert!(recovered.status.is_terminal());
}
