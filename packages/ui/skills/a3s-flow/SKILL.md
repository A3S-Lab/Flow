...
7. Apply one focused edit with `a3s-flow update workflow.json --set-app-name <name>`, `--set-node <id> --config '<json>'`, `--add-node <type> --id <id> [--config '<json>'] [--parent <container-id>]`, `--move-node <id> [--parent <container-id>]`, `--remove-node <id>`, `--add-edge --edge-id <id> --source <id> --target <id>`, `--set-edge <id> --source <id> --target <id>`, or `--remove-edge <id>`. `--parent` is valid for `--add-node` and `--move-node`; it must name an existing `iteration` or `loop`. Omitting it from `--move-node` returns a public node to the top level; use `parentId: null` for the same intent in JSON/NDJSON operations. Moves preserve the node ID, manifest configuration, presentation fields, and unknown extensions. Internal starts must remain under their matching container, and moves that create parent cycles are rejected. To create a container, its matching internal start, and executable children, submit those dependent operations together (the container is intentionally not publishable while incomplete). `iteration-start` must be parented by `iteration`, and `loop-start` by `loop`. `set-edge` changes endpoints (and supplied handles) in place, preserving the stable edge ID and unknown/presentation fields; omitted handles remain unchanged, while `--clear-source-handle`/`--clear-target-handle` explicitly remove them. In JSON/NDJSON operations, use `null` for an explicit handle removal. For a graph edit that needs several intermediate changes, pass `--operations '<json-array>'` so the whole patch is cloned, validated, and atomically replaced once. For large or generated edits, pass `--operations -` and stream one JSON operation per line (NDJSON) on stdin; for generated files, pass `--operations @operations.ndjson` to stream the file without loading it into the command line. Flow applies each operation as it arrives and publishes only after final validation. Use `--if-digest <sha256>` for optimistic concurrency, so a stale agent cannot overwrite a newer file. Add `--dry-run` to inspect the result without writing; rejected or preview-only updates leave the original file unchanged.
   For a hosted adapter or a custom Skill transport that derives an operation idempotency key, canonicalize each parsed operation with `canonicalizeFlowCliWorkflowUpdate` (or the Rust `canonical_workflow_authoring_operation`) before journaling it. This removes insignificant JSON whitespace/key-order differences and materializes only defaults whose wire meaning is identical; graph-dependent checks still happen during update.
...
a3s-flow update workflow.json --set-edge run-step-complete \
  --source run-step --target approval --source-handle success --target-handle in --pretty
a3s-flow update workflow.json --operations '[{"kind":"add-node","id":"each","type":"iteration","configuration":{"start_node_id":"each-start"}},{"kind":"add-node","id":"each-start","type":"iteration-start","parentId":"each"},{"kind":"add-node","id":"process","type":"flow.step","parentId":"each"}]' --pretty
...
a3s-flow update workflow.json --operations @operations.ndjson --if-digest <sha256> --pretty
