...
| Workflow authoring package | `@a3s-lab/flow-ui`, `a3sFlowDagNodeRegistry`, immutable custom-node catalogs, React and Vue `useA3SFlowNode`, `a3s-flow` CLI, `a3s-flow` Skill | `packages/ui/tests/manifest.test.ts`, `packages/ui/tests/node-validity.test.ts`, `packages/ui/tests/hooks.test.tsx`, `packages/ui/tests/dsl.test.ts`, `website/theme/components/WorkflowPlayground.model.test.ts`, `website/theme/components/WorkflowPlayground.custom-nodes.test.ts`, `packages/ui/scripts/smoke-cli.mjs`, `packages/ui/skills/a3s-flow/SKILL.md` | Eighteen public built-in manifests in six groups plus explicitly capability-bound host manifests drive node previews, configuration panels, framework hooks, searchable editor catalogs, publication checks, CLI JSON contracts, and agent authoring instructions. All surfaces consume the same fields, ports, defaults, stable identities, graph validation, and semantic digest rules. |
