...
| --- | --- | --- | --- |
| Event-sourced runs | `FlowEngine`, `FlowEventStore`, `WorkflowRunSnapshot` | `examples/sequential_steps.rs`, `tests/engine.rs` | Run state is projected from append-only typed event envelopes. Engine transitions use expected-sequence appends plus projection validation, so malformed direct writes fail before they can become authoritative history. |
