...
| Projection checkpoints | `FlowProjectionCheckpoint`, `FlowEventStore::{load_checkpoint,save_checkpoint,latest_event}`, `FlowEngine::checkpoint` | `tests/checkpoints.rs`, SQL migration tests | Built-in stores persist disposable materialized snapshots for hot inspection. Flow verifies run identity, latest sequence, and tip event ID before using a checkpoint; missing, corrupt, or stale metadata always falls back to authoritative history replay. Custom stores must opt in explicitly; the default save path fails closed. |
