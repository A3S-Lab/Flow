...
| First-class activities | `RuntimeCommand::ScheduleActivity`, `WorkflowContext::schedule_activity`, `ActivityInvocation`, `FlowEngine::heartbeat_activity`, `FlowEngine::resolve_unknown_activity`, `ActivitySnapshot` | `tests/engine.rs`, `tests/protocol.rs` | Activity attempts persist stable attempt/idempotency/fencing identities before host execution; retries, permanent failures, cancellation, fenced heartbeat checkpoints, and explicit unknown-outcome reconciliation are projected durably. A runtime can stop duplicate side effects after response loss by returning `FlowError::UnknownOutcome`; hosts resolve the attempt with the same fence and idempotency identity, and repeated identical resolutions are idempotent. Existing `FlowRuntime::run_step` implementations remain compatible through the default activity adapter. |
