...
| --- | --- | --- | --- |
| Event-sourced runs | `FlowEngine`, `FlowEventStore`, `WorkflowRunSnapshot` | `examples/sequential_steps.rs`, `tests/engine.rs`, `tests/projection_validation.rs` | Run state is projected from append-only typed event envelopes. Engine transitions use expected-sequence appends plus projection validation, so malformed direct writes fail before they can become authoritative history. Graceful `run_completed` / `run_continued_as_new` fail closed while a step or activity is still in flight (`projection_rejects_run_completed_with_running_step`); immediate host termination remains unconstrained. |
| Workflow DAG import and structural compile | `WorkflowDsl`, `WorkflowDag`, `WorkflowDagPlan`, `WorkflowDslCompatibility` | `examples/workflow_dsl_import.rs`, `tests/workflow_dsl_import.rs`, `tests/fixtures/workflow_dsl_echo.yml` | Complete workflow YAML and extracted graph JSON retain unknown fields, classify DSL compatibility, validate deterministic top-level and iteration/loop scopes, and derive a layout-independent execution digest. Rust and UI share digest format v2 (JavaScript number/UTF-16 ordering, safe-integer and depth guards); edge labels are presentation-only. Empty canvases remain importable drafts but cannot execute. |
| Workflow authoring package | `@a3s-lab/flow-ui`, `a3sFlowDagNodeRegistry`, immutable custom-node catalogs, React and Vue `useA3SFlowNode`, `a3s-flow` CLI, `a3s-flow` Skill, `canonical_workflow_authoring_snapshot`, `canonical_workflow_authoring_operation`, `apply_workflow_authoring_operation`, `WorkflowAuthoringSession` | `packages/ui/tests/manifest.test.ts`, `packages/ui/tests/node-validity.test.ts`, `packages/ui/tests/hooks.test.tsx`, `packages/ui/tests/dsl.test.ts`, `packages/ui/tests/flow-cli.test.ts`, `packages/ui/tests/flow-cli-workflow-stream.test.ts`, `tests/workflow_dsl_authoring.rs`, `tests/workflow_dsl_import.rs`, `website/theme/components/WorkflowPlayground.model.test.ts`, `website/theme/components/WorkflowPlayground.custom-nodes.test.ts`, `packages/ui/scripts/smoke-cli.mjs`, `packages/ui/skills/a3s-flow/SKILL.md` | Eighteen public built-in manifests in six groups plus explicitly capability-bound host manifests drive node previews, configuration panels, framework hooks, searchable editor catalogs, publication checks, and agent authoring instructions. The JSON-first CLI supports safe file CRUD, template import, dry-run, atomic multi-operation patches, bounded NDJSON streams, optimistic base-digest checks, stable-ID-preserving node moves and edge redirects, and scoped `add-node` placement for iteration/loop children while preserving unknown semantic extensions. The Rust boundary applies one strictly bounded operation or incrementally consumes a bounded session over an immutable byte snapshot, exposes separate draft and executable validation, rejects duplicate/deep JSON before canonicalization, and can emit canonical operation bytes for host idempotency keys. It intentionally does not invent product manifests or authorization. All surfaces consume the same stable identities, graph validation, and semantic digest rules. |
| Run inspection | `FlowEngine::list_run_ids`, `FlowEngine::list_snapshots`, `FlowEngine::run_summary`, `FlowEngine::list_open_suspensions`, `FlowEngine::list_due_wakeups`, `FlowEngine::next_wakeup`, `FlowEngine::list_active_hooks`, `FlowEngine::history` | `examples/run_inspection.rs`, `tests/engine.rs`, `tests/store_scheduling_acceleration.rs`, `tests/signals.rs` | Hosts can list sorted run IDs, project snapshots for dashboards, summarize status and actionable suspension counts, list open waits/hooks/signals/retries, find the next timed scheduler wake-up, list resumable external callback hooks, and read raw event history, assembled from bounded pages, for audit or replay debugging. |
| Projection checkpoints and history pages | `FlowProjectionCheckpoint`, `FLOW_PROJECTION_REVISION`, `FlowEventStore::{load_checkpoint,save_checkpoint,latest_event,event_at,list_after,list_page}`, `FlowEngine::{checkpoint,history_page,export_history_pages,export_history_archive,verify_history_archive_seal,seal_history_partition,list_history_partitions}`, `FlowHistoryArchiveSeal`, `FlowHistoryPartition`, `MAX_FLOW_HISTORY_PAGE_SIZE` | `tests/checkpoints.rs`, `tests/history_scale.rs`, SQL migration tests, projection equivalence unit tests | Built-in stores persist disposable materialized snapshots stamped with the current projection revision for hot inspection and expose bounded exclusive-cursor history pages. An older revision is discarded and rebuilt from history (`stale_projection_checkpoint_is_rebuilt_from_history`). `export_history_pages` pins the initial tip, validates contiguous pages, and lets archive/export sinks consume one page at a time without materializing a complete run or chasing new appends. `export_history_archive` returns a tip-pinned ownership seal whose SHA-256 digest over ordered event IDs is independent of page size; `verify_history_archive_seal` rechecks the durable tip. `seal_history_partition` indexes immutable contiguous ranges without rewriting history. Flow verifies run identity, latest sequence, tip event ID, and SHA-256 snapshot digest before using a checkpoint; missing, corrupt, or stale metadata always falls back to authoritative history replay or a bounded indexed tail replay. SQLite and PostgreSQL append transactions advance a validated checkpoint from the new event, keeping steady-state writes independent of full-history length; multi-event tail and derived suspension states are covered by full-vs-incremental equivalence tests. Custom stores must opt in explicitly; the default save path fails closed. |
...
| Durable progress and child operations | `WorkflowProgress`, `ChildOperationReference`, `ExternalDatasetRef`, `ItemAggregateContribution`, `record_progress`, `link_child_operation`, `attach_external_dataset`, `record_item_aggregate` | `tests/durable_operations.rs`, `tests/external_datasets.rs`, `tests/item_aggregates.rs`, `tests/store_reference_integrity.rs`, `tests/sqlite_retention.rs`, `tests/postgres_retention.rs` | Runtime commands and host APIs persist idempotently identified progress updates, child-operation references, content-addressed external dataset refs, and per-item aggregate contributions across replacement workers. Every built-in store requires an optional same-store `flow_run_id` to exist before linking. Child cancellation remains explicitly host-owned. Dataset attachments reject digest/count drift and do not fetch host CAS contents. Item aggregates reject value drift on `(aggregate_id, item_id)`. |
| First-class child workflows | `WorkflowContext::start_child_workflow`, `WorkflowContext::start_child_workflows`, `WorkflowContext::map_child_workflows`, `ChildWorkflowCommand`, `ChildWorkflowMapSnapshot`, `WorkflowContext::child_workflow_outcome`, `ChildWorkflowSnapshot`, `FlowEngineBuilder::with_max_child_workflow_depth`, `MAX_CHILD_WORKFLOW_MAP_SIZE` | `examples/child_workflow.rs`, `examples/child_workflow_batch.rs`, `tests/child_workflows.rs`, `tests/child_workflow_batches.rs`, `tests/child_workflow_maps.rs`, `tests/child_workflow_recovery.rs`, `tests/postgres_child_workflow_interruption.rs`, `tests/child_workflow_validation.rs`, `tests/child_workflow_retention.rs` | Parent-first request events persist generated child identity, exact spec/input, and cancellation policy before cross-stream creation. A due step or activity retry still runs while a child stays open (`due_retry_runs_while_a_child_workflow_is_open`). Appending `ChildWorkflowResolved` forces parent workflow replay so an open timer cannot hide the durable outcome (`child_resolution_wakes_parent_while_another_timer_is_open`). Appending `ChildWorkflowMapCompleted` does the same (`map_completion_wakes_parent_while_another_timer_is_open`); map window advancement still does not force parent replay. A bounded batch validates all definitions before mutation, persists every sibling request before child replay, advances siblings concurrently, and records outcomes in durable request order. Dynamic maps declare an ordered plan up to `MAX_CHILD_WORKFLOW_MAP_SIZE` and activate at most `concurrency` open children at a time, recovering partial windows without duplicate requests and completing only after every planned child resolves; graceful terminals reject an still-open map (`projection_rejects_graceful_terminals_with_an_open_child_workflow_map`). Recovery repairs partial batch requests and either cross-stream crash window without duplicate children. Existing terminal leaves can be resolved by a parent-only build, while missing child creation and active child replay remain fenced by the exact child build. Cancellation propagation follows persisted per-child policy, validation rejects command drift/duplicates/oversized batches/cycles/runaway depth, and retention protects whole ownership components. Externally advanced children require an explicit parent redrive. |
...
| First-class activities | `RuntimeCommand::ScheduleActivity`, `WorkflowContext::schedule_activity_with_timeout`, `ActivityInvocation`, `FlowEngine::heartbeat_activity`, `FlowEngine::resolve_unknown_activity`, `ActivitySnapshot` | `tests/engine.rs`, `tests/protocol.rs`, `tests/crash_recovery.rs`, `tests/postgres_process_recovery.rs`, `tests/postgres_activity_boundary_process_recovery.rs` | Activity attempts persist stable attempt/idempotency/fencing identities and per-attempt deadlines before host execution; timeout, retries, permanent failures, cancellation, fenced heartbeat checkpoints, and explicit unknown-outcome reconciliation are projected durably. A runtime can stop duplicate side effects after response loss or timeout by returning `FlowError::UnknownOutcome`; hosts resolve the attempt with the same fence and idempotency identity, and repeated identical resolutions are idempotent. Crash windows that lose `activity_started` restart a pending activity once; windows that lose `activity_unknown` or `activity_completed` after the host side effect redeliver the same attempt with a rotated lease fence while preserving any durable heartbeat checkpoint. Existing `FlowRuntime::run_step` implementations remain compatible through the default activity adapter. |
...
| Concurrent batch durable steps | `RuntimeCommand::ScheduleSteps`, `WorkflowContext::schedule_steps`, `FlowEvent::StepCancelled` | `examples/batch_steps.rs`, `tests/engine.rs`, `tests/postgres_batch_settlement_interruption.rs` | Step IDs must be stable and unique. Every sibling start is persisted before concurrent execution, each outcome is committed as it settles, due retry siblings fan out together, and a terminal sibling failure durably settles every uncommitted peer before the run terminal event. PostgreSQL-gated interruption mid-settlement recovers without stuck Running siblings or duplicate side effects. Unknown external outcomes remain host-reconciliation work. |
| Content-addressed blob references | `FlowBlobRef`, `FlowBlobEncryption`, `RuntimeCommand::AttachBlobRef`, `FlowEvent::BlobRefAttached`, `WorkflowContext::{attach_blob_ref,blob_ref}`, `$flow_blob_ref` output markers | `tests/blob_refs.rs` | Large step/activity/run payloads can persist host-owned digest/codec/encryption metadata instead of inlining past `MAX_FLOW_EVENT_BYTES`. Attachments are idempotent and reject digest drift; Flow does not fetch CAS bytes. Oversized inline payloads still fail closed. |
| Compensation patterns | `CompensationMarker`, `WorkflowContext::{record_compensation_marker,complete_compensation_marker,compensation_marker,open_compensation_marker_ids}`, `WorkflowContext::schedule_step`, domain-result step outputs | `examples/compensation.rs`, `tests/compensation_markers.rs`, `docs/COOKBOOK.md` | Recoverable business failures can record durable compensation obligations and schedule compensating steps before completion. Markers are bookkeeping only; they do not invent undo logic. Ordinary `drive()` recovery observes tip `CompensationMarkerRecorded` beside an open timer (`compensation_marker_recorded_recovery_wakes_via_drive_while_unscoped_timer_is_open`). |
| Retry policies | `RetryPolicy`, `RetryBackoff`, `StepFailureAction`, `schedule_step_with_retry`, `step_with_retry`, `WorkflowContext::step_failed` | `examples/batch_steps.rs`, `examples/retry_backoff.rs`, `examples/recoverable_step_failure.rs`, `tests/engine.rs`, `tests/scheduler.rs`, `tests/retry_time_bounds.rs`, `tests/projection_validation.rs`, `tests/crash_recovery.rs` | Immediate retries stay in the drive loop; delayed retries suspend until due and still run when another timer, hook, or signal wait is open (`due_retry_runs_while_another_timer_is_open`); fixed histories retain their original encoding, while capped exponential policies apply deterministic full jitter from immutable run/step/attempt identities. Newly computed deadlines anchor at the actual failure clock while retaining a future scheduler cutoff. Exhausted failures fail the run by default or replay to workflow fallback logic when explicitly configured. Unrepresentable UTC delays are rejected before step persistence or execution, replay rejects persisted attempt, budget, deadline, failure-action, or terminal-error drift, and restart reconstructs a missing run-level retry-exhaustion event after a durable final step failure. |
| Timers | `RuntimeCommand::WaitUntil`, `WorkflowContext::wait_until`, `FlowEngine::resume_wait`, `FlowEngine::resume_due_waits` | `examples/scheduler_worker.rs`, `examples/polling_loop.rs`, `tests/scheduler.rs`, `tests/engine.rs`, `tests/worker.rs`, `tests/updates.rs` | Waits do not hold compute; hosts resume them directly or through scheduler work. Existing waits accept matching redelivery after wait completion or run termination without another event. Completion forces workflow replay so another open timer, hook, or signal wait cannot hide the fired timer (`due_wait_wakes_workflow_while_another_timer_is_open`). Forced replay while a wait stays open tolerates `resume_at` recomputation because durable waits already own the bound deadline (`update_force_replay_tolerates_open_wait_resume_at_recomputation`). Changing a deadline requires a new wait id. Redelivery still repairs a missing continue-as-new successor, and reports the active leaf without claiming another task's completion. Concurrent due scans and targeted scheduler tasks report only their completion winner, while compatibility workers acknowledge stale cancelled timer tasks without claiming a resumption. |
| Named workflow signals | `WorkflowSpec::with_signal`, `WorkflowSignal`, `WorkflowContext::wait_for_signal`, `WorkflowContext::signal_payload_as`, `FlowEngine::send_signal`, `FlowTask::SendSignal` | `examples/workflow_signals.rs`, `tests/signals.rs`, `tests/signal_validation.rs`, `tests/signal_recovery.rs`, `tests/signal_observability.rs`, `tests/protocol.rs` | Declared asynchronous messages queue durably before or after a matching wait, pair FIFO by name, and replay from a persisted wait/delivery link. Projection rejects histories that skip an older same-name wait or delivery. Caller-owned signal IDs make identical redelivery idempotent across descendants of the original continuation target; payload/name drift conflicts, cancellation deactivates old waits, graceful `RunCompleted` rejects an open signal wait (`projection_prevents_run_completed_from_abandoning_an_open_signal_wait`), and unsafe continue-as-new cannot drop queued messages. Delivery forces workflow replay so an open timer or hook cannot hide a paired signal (`signal_delivery_wakes_workflow_while_timer_wait_is_open`). Ordinary `drive()` recovery also observes tip `SignalReceived` when a buffered delivery crashes before the forced observation drive (`signal_received_recovery_wakes_via_drive_while_unscoped_timer_is_open`), and tip `SignalWaitCompleted` the same way (`signal_wait_completed_recovery_wakes_via_drive_while_unscoped_timer_is_open`). Direct and queued delivery both repair and drive the active leaf, including a missing continue-as-new successor, with runtime-build admission for non-terminal replay and local-file restart evidence for the post-pairing crash window. Worker outcomes report only the task and stream that commit the durable receipt. Authorization and payload schemas remain host-owned. |
| Typed read-only queries | `WorkflowSpec::with_query`, `QueryInvocation`, `FlowRuntime::run_query`, `FlowEngine::query` | `tests/queries.rs` | Declared query names are pinned with the run. Hosts call `query` with a JSON input; Flow admits the runtime build, rejects undeclared names before dispatch, and answers through `run_query` without appending history. Query input is separate from workflow input so handlers can inspect both. Authorization remains host-owned. |
| Typed synchronous updates | `WorkflowSpec::with_update`, `WorkflowUpdate`, `UpdateInvocation`, `FlowRuntime::run_update`, `FlowEngine::apply_update`, `WorkflowContext::{update,updates}` | `tests/updates.rs` | Declared update names are pinned with the run. Hosts call `apply_update` with a caller-owned `update_id`; Flow admits the runtime build, rejects undeclared names before dispatch, invokes `run_update`, persists `update_applied` with the handler output, then forces one workflow replay so open timer/hook/signal suspensions can observe the new history. Identical retries are idempotent and also force replay so receipt-before-drive crashes recover without re-invoking `run_update`; name or input drift returns `UpdateConflict`. Authorization remains host-owned. |
| Cancellation scopes | `RuntimeCommand::{OpenScope,CompleteScope,CancelScope}`, `FlowEvent::{ScopeOpened,ScopeCompleted,ScopeCancelled}`, `FlowEngine::cancel_scope`, `WorkflowContext::{open_scope,complete_scope,cancel_scope,has_scope,scope_cancelled}` | `tests/cancellation_scopes.rs`, `tests/sqlite_scheduled_wakeups.rs`, `tests/postgres_scheduled_wakeups.rs` | Nested scopes are durable history. Timer waits inherit the innermost open scope at creation. Host or workflow cancellation of a scope cancels owned waits, structured selects opened inside that tree, and still-open descendant scopes without terminating the run. A timer owned by a descendant that has already completed stays indexed (`sqlite_scope_cancel_keeps_a_wait_owned_by_a_completed_child`). Identical host retries are idempotent, force replay when `ScopeCancelled` is durable but the run is still non-terminal, and reason drift returns `ScopeConflict`. Host cancel also forces replay beside an unscoped open timer (`scope_cancel_wakes_workflow_while_unscoped_timer_wait_is_open`); ordinary `drive()` recovery observes tip `ScopeOpened` / `ScopeCancelled` / `ScopeCompleted` the same way (`workflow_scope_opened_recovery_wakes_via_drive_while_unscoped_timer_is_open`, `workflow_scope_cancelled_recovery_wakes_via_drive_while_unscoped_timer_is_open`, `workflow_scope_completed_recovery_wakes_via_drive_while_unscoped_timer_is_open`). Run cancellation marks every open scope cancelled during projection. |
| Structured select/race/join | `RuntimeCommand::Select`, `SelectMode`, `SelectArm`, `FlowEvent::{SelectCreated,SelectCompleted}`, `WorkflowContext::{select,join,select_winner,select_joined}` | `tests/select.rs`, `tests/join.rs` | A select records at least two timer/signal arms atomically under `SelectMode::Race` (default) or `JoinAll`. Race: the first completed arm wins and sibling waits are cancelled. Join-all: every arm must complete and siblings are not cancelled early. Redrive while open is idempotent; arm identity, signal name, kind, and mode drift are rejected, while timer `resume_at` recomputation is tolerated because durable waits already own the bound times (`join_all_completes_only_after_every_arm_finishes`, `open_select_rejects_arm_identity_drift`). Losing `SelectCompleted` after a durable winning arm recovers with exactly one completion (`select_recovers_after_select_completed_persistence_is_lost`). |
| External callbacks | `RuntimeCommand::CreateHook`, `WorkflowContext::create_hook_with_metadata`, `WorkflowContext::hook_disposed`, `HookMetadata`, `HookCallbackRoute`, `ActiveHookSnapshot`, `FlowEventStore::find_active_hooks_by_token`, `FlowEventStore::list_active_hooks`, `resume_hook`, `resume_hook_by_token`, `dispose_hook`, `dispose_hook_by_token` | `examples/hook_approval.rs`, `examples/hook_disposal.rs`, `tests/context.rs`, `tests/engine.rs`, `tests/hook_idempotency.rs`, `tests/store_query_acceleration.rs`, `tests/sqlite_active_hooks.rs`, `tests/postgres_active_hooks.rs`, `tests/worker.rs` | Stable run/hook-identified resume and disposal are idempotent for matching durable redelivery across concurrent writers, interrupted drive, terminal completion, and a missing continue-as-new successor; payload drift and opposite resolutions return typed `HookConflict`. Receipt and disposal force workflow replay so an open timer or signal wait cannot hide the callback (`hook_receipt_wakes_workflow_while_timer_wait_is_open`, `hook_disposal_wakes_workflow_while_timer_wait_is_open`). Worker outcomes report only the task that commits the receipt or disposal, including when token lookups race, while `run_ids` follows the active leaf. Public-token lookup remains active-only. Active tokens are unique across active runs; SQL stores use an ORM-managed, migration-backfilled projection for parameterized lookup and enforce ownership under concurrent writers. Token lookup/conflict errors retain typed values but redact them from `Display` and `Debug`; typed metadata helpers standardize audit and callback routing fields. |
| Task management | `FlowTaskDispatcher`, `BootFlowTaskManager`, `BootFlowTaskPolicy`, `BootFlowTaskDeduplication`, `FlowTaskQueue`, `enqueue_for_partition`, `with_partition_fairness`, `FlowWorker`, `FlowWorkerCapabilities` | `tests/boot.rs`, `examples/boot_task_policy.rs`, `examples/scheduler_worker.rs`, `examples/task_queue_durability.rs`, `examples/postgres_task_queue_durability.rs`, `tests/worker.rs`, `tests/signals.rs`, `tests/queue_time_bounds.rs`, `tests/postgres_process_recovery.rs`, `tests/postgres_activity_boundary_process_recovery.rs` | A3S Boot is the recommended application task manager and owns queue processors, job state, lifecycle, retry, timeout, retention, logical deduplication, and shutdown. Flow maps stable task targets to typed Boot options, hashes callback tokens in deduplication metadata, deduplicates signals by run/signal identity, and deduplicates scheduled work by run ID while retaining the latest active successor. `DriveRun` outcomes report the active continuation leaf while preserving the submitted task target for correlation. Flow-owned queues remain embedded/compatibility primitives; their leases heartbeat with rotating fencing tokens, reject stale completion after reclaim, preserve lease-age ordering at extreme UTC cutoffs, support idempotent dead-letter redrive, and let compatibility hosts cap a drain with `FlowWorker::run_until_idle_bounded(limit)` before yielding. `FlowWorkerCapabilities` provides a versioned fail-closed handshake for task kinds and kernel guarantees before leasing. A PostgreSQL subprocess gate kills a worker before step completion, then proves lease expiry, reconnect, same-attempt replay, and one logical side effect. |
...
| Scheduling | `ScheduledWakeup`, `FlowEventStore::list_due_wakeups`, `FlowEventStore::next_scheduled_wakeup`, `FlowEngine::resume_scheduled_run`, `FlowTask::ResumeScheduledRun`, `FlowScheduler::next_wakeup`, `FlowScheduler::next_wakeup_delay`, `FlowScheduler::enqueue_due_work` | `examples/scheduler_worker.rs`, `tests/scheduler.rs`, `tests/boot.rs`, `tests/store_scheduling_acceleration.rs`, `tests/sqlite_scheduled_wakeups.rs`, `tests/postgres_scheduled_wakeups.rs`, `tests/worker.rs` | Scheduler reports the next timed wake-up, discovers due waits and retries in one store query, groups them into one task per affected run, and gives hosts a sleep-friendly delay. Workers replay only the target run, avoid a second global due query, drive due retry siblings together, repair committed continuation boundaries on task redelivery, report the active leaf, and report only wait completions committed by that targeted task. Direct open-timer redelivery is rejected until the persisted deadline; terminal redelivery remains idempotent. The public targeted engine call retains its due-at-start return contract. SQL stores use indexed ORM projections; other stores retain replay-compatible defaults. |
| Local and shared durability | `LocalFileEventStore`, `InMemoryEventStore`, `FlowRunShardLayout`, `ShardedFlowEventStore`, `SqliteEventStore`, `PostgresEventStore`, `FlowHistoryRetentionPolicy`, `LocalFileFlowTaskQueue`, `PostgresFlowTaskQueue` | `examples/local_file_durability.rs`, `examples/sqlite_durability.rs`, `examples/sqlite_retention.rs`, `examples/postgres_durability.rs`, `examples/local_retention.rs`, `tests/store_reference_integrity.rs`, `tests/child_workflow_retention.rs`, `tests/sqlite_retention.rs`, `tests/postgres_retention.rs`, `tests/sqlite_active_hooks.rs`, `tests/postgres_active_hooks.rs`, `tests/sqlite_scheduled_wakeups.rs`, `tests/postgres_scheduled_wakeups.rs`, `tests/postgres_process_recovery.rs`, `tests/postgres_activity_boundary_process_recovery.rs`, `tests/run_sharding.rs`, `tests/scale_slos.rs`, `tests/postgres_chaos.rs` | Local JSONL, SQLite, and PostgreSQL stores share one retention planner that deletes complete eligible terminal components only and protects external child references, first-class child ownership, and continuation links. In-memory and local-file stores can partition run histories across physical shards via `FlowRunShardLayout` while keeping linked-run and hook-token checks store-wide. SQL stores additionally use A3S ORM transactions, typed decoding, checksummed migrations, durable audit holds, checksum tombstones, and transactionally maintained active-hook and scheduled-wakeup projections. Partial event-stream compaction is never performed. |
| Observability | `FlowEventObserver`, `FanoutFlowEventObserver`, `A3sFlowEventBridge`, `A3sFlowEvent`, `A3sEventBusFlowEventSink`, `InMemoryFlowEventObserver`, `LocalFileA3sFlowEventSink`, `FlowTraceContext`, `with_trace_context`, `FlowVisibilityProjection`, `FlowEngine::visibility_projection` | `examples/observer_bridge.rs`, `examples/observer_fanout.rs`, `examples/local_audit_log.rs`, `tests/engine.rs`, `tests/trace_context.rs`, `tests/visibility_projection.rs` | Observers mirror committed events after store append; fan-out observers deliver concurrently and isolate sink panics, while the engine bounds observer delay and keeps durable transitions authoritative. Bridge records expose A3S event keys, safe metric labels, local JSONL audit records, optional A3S Event publishing, and stable step/activity attempt correlation for logs/traces (`attempt`, `attempt_id`, `idempotency_key`) without adding high-cardinality metric dimensions. Ambient W3C Trace Context binds to host drives via `with_trace_context`, surfaces on invocations, and mirrors `traceparent` / `tracestate` on bridge/sink metadata without entering replay history. `visibility_projection` returns a tip-anchored, schema-versioned index contract rebuildable from history or tip-validated snapshots without copying workflow payloads. Terminal events release bridge workflow-identity cache entries; the local sink preserves complete unterminated records, repairs only malformed unterminated tails, and rejects terminated or interior corruption before append. |
...
## Streaming authoring test matrix

The stream path is correct only when the following invariants hold. Tests are
organized by the failure they prevent, not by CLI command count:

| Invariant | Required cases | Evidence |
| --- | --- | --- |
| Framing is lossless | Strict UTF-8 split across byte and text chunks, LF/CRLF, blank lines, final line without newline, and no stranded decoder state | `flow-cli-workflow-stream.test.ts` |
| Input is bounded | Empty stream, malformed UTF-8/JSON, malformed workflow files, unknown operation fields, one-line byte limit (including ignored whitespace), and the shared 10,000-operation limit across JSON arrays, direct typed calls, and NDJSON before the extra operation is applied | `flow-cli-workflow-stream.test.ts` and CLI error tests |
...
| `child_workflow_batch` | Present | Persist every sibling identity before concurrently advancing a bounded child fan-out, then fold terminal outcomes in stable ordinal order. |
| `child_workflow_map` | Present | Persist an ordered map plan, activate a concurrency window, advance without silently dropping planned children, and reject plan drift. |
...
3. **Production storage and task management**
   ...
   - Keep first-class child requests parent-authoritative and child creation and
     ...
     depth/cycle guards, explicit parent redrive, and linked retention. Keep
     batch requests bounded, fully validated and persisted before child replay;
     advance siblings concurrently while committing parent outcomes in durable
     request order, including recovery from a partially appended request set.
...
5. **Workflow authoring ergonomics**
   - Keep the 18 public node manifests, six authoring groups, configuration
     ...
     snapshot), registry-driven container parent placement and stable-ID-preserving
     node moves, optimistic base-digest checks, and Skill instructions aligned with the
