...
| Structured select/race | `RuntimeCommand::Select`, `SelectArm`, `FlowEvent::{SelectCreated,SelectCompleted}`, `WorkflowContext::{select,select_winner}` | `tests/select.rs` | A select records at least two timer/signal arms atomically. The first completed arm wins; sibling waits are cancelled. Redrive while open is idempotent; arm drift is rejected. |
