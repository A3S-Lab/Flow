...
| First-class activities | `RuntimeCommand::ScheduleActivity`, `WorkflowContext::schedule_activity`, `ActivityInvocation`, `FlowEngine::heartbeat_activity`, `ActivitySnapshot` | `tests/engine.rs` | Activity attempts persist stable attempt/idempotency/fencing identities before host execution; retries, permanent failures, cancellation, and fenced heartbeat checkpoints are projected durably. Existing `FlowRuntime::run_step` implementations remain compatible through the default activity adapter. |
