...
| Projection checkpoints | `FlowProjectionCheckpoint`, `FlowEventStore::{load_checkpoint,save_checkpoint,latest_event,event_at,list_after}`, `FlowEngine::checkpoint` | `tests/checkpoints.rs`, SQL migration tests | Built-in stores persist disposable materialized snapshots for hot inspection. Flow verifies run identity, latest sequence, tip event ID, and SHA-256 snapshot digest before using a checkpoint; missing, corrupt, or stale metadata always falls back to authoritative history replay or a bounded indexed tail replay. Custom stores must opt in explicitly; the default save path fails closed. |
