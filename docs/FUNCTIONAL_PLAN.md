...
| Projection checkpoints and history pages | `FlowProjectionCheckpoint`, `FlowEventStore::{load_checkpoint,save_checkpoint,latest_event,event_at,list_after,list_page}`, `FlowEngine::{checkpoint,history_page,export_history_pages}`, `MAX_FLOW_HISTORY_PAGE_SIZE` | `tests/checkpoints.rs`, SQL migration tests, projection equivalence unit tests | Built-in stores persist disposable materialized snapshots for hot inspection and expose bounded exclusive-cursor history pages. `export_history_pages` pins the initial tip, validates contiguous pages, and lets archive/export sinks consume one page at a time without materializing a complete run or chasing new appends. Flow verifies run identity, latest sequence, tip event ID, and SHA-256 snapshot digest before using a checkpoint; missing, corrupt, or stale metadata always falls back to authoritative history replay or a bounded indexed tail replay. SQLite and PostgreSQL append transactions advance a validated checkpoint from the new event, keeping steady-state writes independent of full-history length; multi-event tail and derived suspension states are covered by full-vs-incremental equivalence tests. Custom stores must opt in explicitly; the default save path fails closed. |
