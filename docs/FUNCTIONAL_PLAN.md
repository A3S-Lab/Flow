...
| First-class child workflows | `WorkflowContext::start_child_workflow`, `WorkflowContext::start_child_workflows`, `ChildWorkflowCommand`, `WorkflowContext::child_workflow_outcome`, `ChildWorkflowSnapshot`, `FlowEngineBuilder::with_max_child_workflow_depth` | `examples/child_workflow.rs`, `examples/child_workflow_batch.rs`, `tests/child_workflows.rs`, `tests/child_workflow_batches.rs`, `tests/child_workflow_recovery.rs`, `tests/child_workflow_validation.rs`, `tests/child_workflow_retention.rs` | Parent-first request events persist generated child identity, exact spec/input, and cancellation policy before cross-stream creation. A bounded batch validates all definitions before mutation, persists every sibling request before child replay, advances siblings concurrently, and records outcomes in durable request order. Recovery repairs partial batch requests and either cross-stream crash window without duplicate children. Existing terminal leaves can be resolved by a parent-only build, while missing child creation and active child replay remain fenced by the exact child build. Cancellation propagation follows persisted per-child policy, validation rejects command drift/duplicates/oversized batches/cycles/runaway depth, and retention protects whole ownership components. Externally advanced children require an explicit parent redrive. |
