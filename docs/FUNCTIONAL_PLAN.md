...
| Concurrent batch durable steps | `RuntimeCommand::ScheduleSteps`, `WorkflowContext::schedule_steps`, `FlowEvent::StepCancelled` | `examples/batch_steps.rs`, `tests/engine.rs` | Step IDs must be stable and unique. Every sibling start is persisted before concurrent execution, each outcome is committed as it settles, due retry siblings fan out together, and a terminal sibling failure durably settles every uncommitted peer before the run terminal event. Unknown external outcomes remain host-reconciliation work. |
