...
`ScopeCancelled` (`workflow_scope_cancelled_recovery_wakes_via_drive_while_unscoped_timer_is_open`).
...
configuration, while all direct operation APIs enforce the one-megabyte
per-operation budget at runtime. Canonicalization therefore cannot silently
choose a last duplicate field, which keeps digest inputs unambiguous across
Rust, TypeScript, and hosted adapters.
