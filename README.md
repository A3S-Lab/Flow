...
| CLI          | JSON-first `a3s-flow` commands for node discovery plus file CRUD: `create`, `read`, `update`, `delete`, `validate`, `compile`, and `digest`; updates include stable-ID-preserving edge redirects, batch or NDJSON-streamed patches, optimistic digest checks, dry-run, atomic writes, and final candidate revalidation                                                                                         |
