...
`--operations` when several graph changes must be committed together. Node
configuration is JSON and only manifest-owned fields are changed; unknown
semantic extensions and presentation fields remain intact. The candidate is
validated before the atomic replace, so a rejected edit leaves the original file
