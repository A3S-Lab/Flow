...
| `update <file>`         | Apply one graph/app edit atomically        | Changed IDs plus the new validated summary                     |
...
`update` accepts exactly one focused edit per invocation. Node configuration is
JSON and only manifest-owned fields are changed; unknown semantic extensions and
presentation fields remain intact. The candidate is validated before the atomic
replace, so a rejected edit leaves the original file unchanged.
