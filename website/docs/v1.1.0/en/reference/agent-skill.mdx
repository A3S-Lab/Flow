...
The workflow begins with `a3s-flow nodes --pretty`, followed by `a3s-flow node <type> --pretty` for each candidate. New files come from `a3s-flow create`, and existing files are inspected with `a3s-flow read`. New nodes come from `a3s-flow new`, preserving manifest defaults. File edits use one `a3s-flow update` operation at a time; the CLI validates the candidate and atomically replaces the file only when it remains executable. If validation reports issues, it repairs the named paths or edges and repeats the entire sequence. Use `a3s-flow delete --force` only when the user explicitly requests removal.
