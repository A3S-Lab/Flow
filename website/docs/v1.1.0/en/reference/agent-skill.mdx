...
The workflow begins with `a3s-flow nodes --pretty`, followed by `a3s-flow node <type> --pretty` for each candidate. New files come from `a3s-flow create`, and existing files are inspected with `a3s-flow read`. New nodes come from `a3s-flow new`, preserving manifest defaults. File edits use one `a3s-flow update` operation at a time; generated edits can stream NDJSON through `--operations -`, and `--if-digest` prevents a stale agent from overwriting a newer base. The CLI applies streamed operations incrementally, validates the final candidate, and atomically replaces the file only when it remains executable. If validation reports issues or the base digest conflicts, it repairs/rebases the named paths or edges and repeats the sequence. Use `a3s-flow delete --force` only when the user explicitly requests removal.
