...
Cancelling a durable run usually has two parts. The control plane records a stop request, then workflow code reverses external state it already created. Flow 1.1 provides `request_cancellation()` for this path. Cleanup steps enter history like ordinary steps and recover after process loss.
