...
Skill 会先运行 `a3s-flow nodes --pretty`，再用 `a3s-flow node <type> --pretty` 查看候选节点。新文件通过 `a3s-flow create` 生成，已有文件用 `a3s-flow read` 检查。新节点通过 `a3s-flow new` 生成，避免遗漏默认值。文件修改每次使用一个 `a3s-flow update` 操作；需要调整连线端点时使用 `--set-edge <id> --source <id> --target <id>`，这样会保留稳定 ID 和扩展字段。公开节点可以用 `--add-node ... --parent <container-id>` 放入已有遍历或循环，也可以用 `--move-node <id> [--parent <container-id>]` 在保留稳定 ID 和字段的情况下移动；省略 `--parent` 可将公开节点移回顶层。新容器、匹配的内部起点和可执行子节点应放在同一个 `--operations` 数组或 NDJSON 流中（JSON operation 使用 `parentId`）。生成的较大修改可通过 `--operations -` 以 NDJSON 流式输入，也可以使用 `--operations @operations.ndjson` 直接流式读取文件，避免把大 patch 放进命令行参数。`--if-digest` 用于防止过期 Agent 覆盖新的基线。CLI 会逐条应用 operation，完整校验候选文档后才原子替换。遇到校验问题或基线摘要冲突时，应先修复或重新基于最新文档生成修改，再执行完整序列。只有用户明确要求时才使用 `a3s-flow delete --force`。

这套顺序有两个用途。第一，节点 manifest 与当前安装版本保持一致。第二，语义摘要只在可执行图已经通过校验并得到稳定顺序后产生。Agent 应在最终说明中列出修改过的节点与边、编译顺序、摘要以及仍由宿主负责的任务处理器。

如果 operation 要进入托管日志，先使用
`canonicalizeFlowCliWorkflowUpdate`（或 Flow 的 Rust
`canonical_workflow_authoring_operation`）规范化，再计算幂等键。这样只是
空格或键顺序不同的 JSON 会得到同一份 operation 字节表示。
