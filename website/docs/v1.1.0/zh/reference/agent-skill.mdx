...
Skill 会先运行 `a3s-flow nodes --pretty`，再用 `a3s-flow node <type> --pretty` 查看候选节点。新文件通过 `a3s-flow create` 生成，已有文件用 `a3s-flow read` 检查。新节点通过 `a3s-flow new` 生成，避免遗漏默认值。文件修改每次使用一个 `a3s-flow update` 操作；生成的较大修改可通过 `--operations -` 以 NDJSON 流式输入，`--if-digest` 用于防止过期 Agent 覆盖新的基线。CLI 会逐条应用 operation，完整校验候选文档后才原子替换。遇到校验问题或基线摘要冲突时，应先修复或重新基于最新文档生成修改，再执行完整序列。只有用户明确要求时才使用 `a3s-flow delete --force`。
