...
This page follows one complete executable path. The workflow schedules a step, the step output enters history, and replay completes the run. It uses the in-memory store so you can verify the runtime boundary and event order first.
...
The final snapshot should be `Completed`, with output from the committed `greet` step. The workflow ran twice for a reason.
