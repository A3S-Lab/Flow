...
const EXECUTION_DIGEST_DOMAIN: &[u8] = b"a3s.flow.workflow_dsl.execution.v1\0";
const GRAPH_EXECUTION_DIGEST_DOMAIN: &[u8] = b"a3s.flow.workflow_dag.execution.v1\0";
...
fn stable_digest(domain: &[u8], value: &Value) -> Result<String, WorkflowDslError> {
    let encoded = serde_json::to_vec(value).map_err(|error| WorkflowDslError::Serialization {
        message: error.to_string(),
    })?;
